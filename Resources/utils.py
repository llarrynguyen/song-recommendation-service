...
class Utils:
    ...
    def get_function_switch(self, key, map_of_functions):
        '''
    ...
    def append_collumn_to_matrix(self, M):
        '''
    ...
    def append_row_to_matrix(self, M):
        '''
    ...
    def get_user_item_matrix(self, dataset):
        ...
        for row in dataset:
            ...
            M, user_index, item_index = self.get_user_item_index(
                user_dict, item_dict, user_elem, item_elem, M)
    ...
    def euclidean_with_nan(self, x, y):
        x = np.array(x)
        y = np.array(y)
    ...
    def nearest_neighbor(self, M, index, forb_ind):
        item = M[index]
        search_M = M
        min_dist = np.Inf
        min_ind = 0
        for it in range(len(search_M)):
            if it in forb_ind:
                continue
            dist = self.euclidean_with_nan(item, search_M[it])
            if dist < min_dist:
                min_ind = it
                min_dist = dist
        return search_M[min_ind], min_ind

    def replace_missing_values_nn(self, M):
        for i in range(len(M)):
            if np.any(np.isnan(M[i])):
                ...
                while not np.all(union):
                    neighbor, n_ind = self.nearest_neighbor(M, i, forb_ind)
